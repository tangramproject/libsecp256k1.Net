...
namespace Libsecp256k1Zkp.Net
{
    public class Secp256k1 : IDisposable
    {
        ...
        /// <returns></returns>
        public byte[]? PublicKeyCombine(IEnumerable<byte[]> pubKeys)
        {
            var pk = new byte[64];
            var bytesEnumerable = pubKeys as byte[][] ?? pubKeys.ToArray();
            var ptrs = new IntPtr[bytesEnumerable.Length];

            for (var i = 0; i < bytesEnumerable.Length; i++)
            {
                var ptr = Marshal.AllocHGlobal(bytesEnumerable[i].Length);
                Marshal.Copy(bytesEnumerable[i], 0, ptr, bytesEnumerable[i].Length);
                ptrs[i] = ptr;
            }

            return secp256k1_ec_pubkey_combine(Context, pk, ptrs, bytesEnumerable.Length) == 1 ? pk : null;
        }
